DROP FUNCTION IF EXISTS public.create_guest_gc_account(text, text, text, text);

CREATE FUNCTION public.create_guest_gc_account(
  p_company_name text,
  p_company_address text,
  p_project_name text,
  p_project_address text
)
RETURNS TABLE(created_company_id uuid, created_project_id uuid)
LANGUAGE plpgsql
SECURITY DEFINER
