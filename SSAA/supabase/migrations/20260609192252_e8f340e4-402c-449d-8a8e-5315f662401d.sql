DROP FUNCTION IF EXISTS public.get_primary_account_holder_for_company(uuid);
...
BEGIN
  ...
  IF v_is_guest IS TRUE THEN
    RETURN QUERY
    SELECT p.id, p.user_id, p.full_name, p.email, c.id, c.name, c.company_type, ur.permission_level
