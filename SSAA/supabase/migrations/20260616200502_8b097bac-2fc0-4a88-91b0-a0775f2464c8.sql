...
BEGIN
  ...
  -- Build the date set for this message: only days that ACTUALLY entered
  -- this state in the current update wave (within the last 30 seconds).
  -- INSERTs and missing request_group_id fall back to the single row.
  ...
  -- Dedupe key uses kind + group + sorted date-set so a later wave
  -- (e.g. confirm-after-edit on different days) is not suppressed by an
  -- earlier different-kind or different-date message.
  v_date_key := COALESCE((
