...
-- Trigger: block project inserts past cap
...
BEGIN
  ...
  FROM public.companies c
  LEFT JOIN public.company_subscriptions cs ON cs.company_id = c.id AND cs.status = 'active'
...
-- Trigger: block employee inserts past cap
...
DROP TRIGGER IF EXISTS enforce_employee_limit_trigger ON public.employees;
